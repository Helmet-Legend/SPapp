...
function formatScenario(texte) {
    return echapperHTML(texte)
        .replace(/\*\*(.*?)\*\*/g, '<strong style="color: #6A1B9A;">$1</strong>')
        .replace(/^### (.*$)/gm, '<h4 style="color: #8E24AA; margin-top: 20px;">$1</h4>')
        .replace(/^## (.*$)/gm, '<h3 style="color: #6A1B9A; margin-top: 25px; border-bottom: 2px solid #E1BEE7; padding-bottom: 5px;">$1</h3>')
        .replace(/^# (.*$)/gm, '<h2 style="color: #4A148C; margin-top: 25px;">$1</h2>')
